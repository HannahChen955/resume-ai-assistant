...
embedding_model = config.get("DASHSCOPE_EMBEDDING_MODEL")
llm_model = config.get("DASHSCOPE_LLM_FIELD_EXTRACT_MODEL")
ie_model = config.get("DASHSCOPE_LLM_IE_EXTRACT_MODEL")
...
"""
    try:
        ...
        payload = {
            "model": llm_model,
            "input": {"prompt": prompt},
            "parameters": {"max_tokens": 1000, "temperature": 0}
        }
        response = requests.post(url, headers=headers, json=payload, timeout=60)
        content = response.json().get("output", {}).get("text", "[]")
        content = re.sub(r"^```(json)?|```$", "", content.strip())
...
def main():
    ...
    for filename in tqdm(files, desc="📄 正在提取简历"):
        try:
            ...
            name_clean = sanitize_filename_part(name)
            position_clean = sanitize_filename_part(position)
